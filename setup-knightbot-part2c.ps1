...
Write-Host "`n🛡️ KNIGHTBOT PART 2C - Main Page + Final Setup`n" -ForegroundColor Cyan
    ...
    <div className="flex flex-col h-screen">
      ...
      {isVoiceMode && (
        <VoiceOverlay
          ...
          onToggle={() => { handleInterrupt(); setVoiceMode(false); }}
...
Write-Host "✓ next-env.d.ts" -ForegroundColor Green

# ─── UPDATE START.PS1 (Add frontend) ───
...
if ($lmStudio.TcpTestSucceeded) {
  Write-Host " ✓ LM Studio detected on port 1234" -ForegroundColor Green
} else {
  Write-Host " ⚠ LM Studio not running - start it and load a model" -ForegroundColor Yellow
}
...
Write-Host " ✓ Knight API starting" -ForegroundColor Green
...
Write-Host " ✓ Parakeet STT starting" -ForegroundColor Green
...
Start-Sleep -Seconds 2
Write-Host " ✓ Chatterbox TTS starting" -ForegroundColor Green
...
Write-Host "✓ start.ps1 (updated with frontend)" -ForegroundColor Green

# ─── STOP.PS1 (Complete shutdown) ───
...
Write-Host "`n✓ KnightBot stopped`n" -ForegroundColor Green
'@ | Set-Content "$BaseDir\stop.ps1" -Encoding UTF8

Write-Host "✓ stop.ps1 (updated)" -ForegroundColor Green
...
try {
  ...
  Write-Host " ✗ Node.js not found. Install Node 18+ first." -ForegroundColor Red
  exit 1
}
...
- **Knight API**: Python backend with LLM integration (port 8100)
- **STT**: Parakeet speech-to-text server (port 8070)
- **TTS**: Chatterbox text-to-speech server (port 8060)
- **Vector DB**: Qdrant (port 6333)
- **Memory**: Mem0 context management (port 8050)
- **LLM**: LM Studio local models (port 1234)

## Voice Shortcuts

- **Ctrl+V** - Toggle voice mode
- **Escape** - Interrupt/stop everything
- **Space** - Stop recording (in voice mode)

## Troubleshooting

### LM Studio not detected
Make sure LM Studio is running and a model is loaded before starting KnightBot.

### Microphone not working
Check browser permissions. In the browser console, ensure microphone access is granted.

### Services not starting
Check that all required ports (3000, 8100, 8070, 8060, 6333, 8050, 1234) are available.
...
'@ | Set-Content "$BaseDir\README.md" -Encoding UTF8

Write-Host "✓ README.md" -ForegroundColor Green

Write-Host "`n╔════════════════════════════════════════════════════════════╗" -ForegroundColor Green
Write-Host "║ ✓ KNIGHTBOT PART 2C COMPLETE                              ║" -ForegroundColor Green
Write-Host "╚════════════════════════════════════════════════════════════╝`n" -ForegroundColor Green

Write-Host "Files created:" -ForegroundColor Cyan
Write-Host " • src/app/page.tsx (React main component)" -ForegroundColor White
Write-Host " • next-env.d.ts (TypeScript definitions)" -ForegroundColor White
Write-Host " • start.ps1 (Startup script)" -ForegroundColor White
Write-Host " • stop.ps1 (Shutdown script)" -ForegroundColor White
Write-Host " • install.ps1 (Installation script)" -ForegroundColor White
